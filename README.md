...
- 📊 **任务状态外显（对标 Codex）**：思考中（浅蓝）→ 工作中（蓝光，机器人目镜发光呼吸）→ 完成后进入**待复核**（琥珀色，持续显示、点击宠物清除并聚焦窗口）；失败 ❌ 红光 + 安慰，后台任务完成 ❗ 提醒，按 Esc 打断收起
- 🖱️ **点击聚焦**：单击宠物把 Kimi Code 窗口带到前台（对标 Codex 宠物；设置里可切回互动模式）
- 🤖 **科技感机器人**：默认形象是银色小机器人（青色发光目镜/天线/耳机），另有小猫 / 小狗 / 史莱姆 / 小兔 / 外星人 / 拼豆鸭（点几下拼豆 → 熨烫 → 撕纸诞生 🦆）、MBTI 人格像素系列十六型（每型专属道具和台词，领走你自己那一型），支持导入自定义图片
...
- 🖱️ **经典桌宠体验**：悬浮置顶、拖拽（身体会倾斜回弹）、点击互动（跳跃/转圈/爱心粒子）、随机散步；眼睛会跟着你的鼠标转，hover 有回应，待机时会歪头/伸懒腰/张望，5 分钟不理它会打盹 💤；气泡文案按形象区分性格
...
在 Kimi Code 里执行（CLI 和 Desktop 桌面版都可以，直接在对话输入框输入）：
...
> Desktop 桌面版：如果 app 是从 Dock / Finder 启动的，系统 PATH 里通常没有 node，插件内置了 `mcp/run-node.sh` 会自动探测常见安装路径（homebrew / nvm / volta / fnm 等），无需手动配置。点击宠物聚焦窗口时优先唤起 Kimi Code 桌面版。
...
| 🧩 MBTI 人格系列（16 型全） | 原创像素风，各型专属道具与台词：NT 夜幕军师/奇思博士/破阵统帅/点子王 · NF 拾梦旅人/星灯隐士/篝火团长/彩虹弹弹 · SJ 暖灯管家/方格哨兵/号令队长/甜甜班长 · SP 火花玩家/慢画旅人/扳手游侠/闪光爱豆 |
...
**Q：宠物被我不小心关了？**
托盘图标还在就没事：托盘菜单 → 勾回「显示宠物」（或等下次会话 / 工具调用时自动拉起）。想彻底退出：托盘菜单 → 退出。

**Q：多个 Kimi Code 会话会怎样？**
它们共享同一只宠物。任务状态以“最近事件”为准；手动 `pet_say` 等操作各会话都能用。

**Q：Windows 能用吗？**
宠物 app 本身跨平台，但目前只在 macOS 上实测过：macOS（CLI + Desktop）完整支持；Windows 的 MCP server 和 hooks 依赖 POSIX `sh`（用于自动探测 node 路径），需要 Git Bash / WSL 环境，暂未完成适配。

**Q：状态徽标都代表什么？**

| 徽标 | 含义 |
| --- | --- |
| 🔵 思考中 / 工作中 | Kimi Code 正在思考或执行（浅蓝=思考，蓝=干活，机器人目镜会发光） |
| 🟠 待复核 | 本轮已完成，等你查看结果——点击宠物即可清除并聚焦窗口 |
| 🔴 出错 | 本轮失败（宠物会安慰你），几秒后自动收起 |
| 🟠 提醒 | 后台任务完成，来看看结果 |
